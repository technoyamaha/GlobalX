...
// import { cryptoPlans } from "../constants/cryptoPlans.js";
...
//     };
//   } catch (error) {
//     console.error("Error loading crypto plans:", error);
//     throw Error("Unable to get user data!");
//   } finally {
...
// const distributeSignupBonus = async (req, res) => {
...
//   -- Get the amount from the FIRST successful recharge
//   (
//     SELECT rr.money
//     FROM recharge rr
//     WHERE rr.user_id = u.id AND rr.status = 1
//     ORDER BY rr.time ASC
//     LIMIT 1
//   ) AS wallet_balance,

//   -- Sum of all signup bonuses paid
//   COALESCE(SUM(b.amount), 0) AS total_paid,

//   -- Number of signup bonuses paid
//   COUNT(b.id) AS days_paid

// FROM users u

// -- Join bonuses table to get signup bonuses
// LEFT JOIN bonuses b
//   ON u.id = b.user_id
//  AND b.type = 'signup'
//  AND b.status = 1
...
//     // Calculate start and end of today in ms timestamp
...
//       const potentialNewTotal = user.total_paid + dailyBonus;
//       const bonusToPay =
//         potentialNewTotal > 100 ? 100 - user.total_paid : dailyBonus;

//       if (bonusToPay <= 0) continue;
...
//           WHERE user_id = ?
//             AND type = 'signup'
//             AND DATE(FROM_UNIXTIME(CAST(created_at AS UNSIGNED) / 1000)) = CURDATE() FOR UPDATE`,
//         [user.id]
//       );
...
//         await connection.query(
//           `INSERT INTO bonuses
//             (user_id, type, amount, description, status, created_at, updated_at)
//            VALUES (?, 'signup', ?, 'Daily signup bonus credit', 1, ?, ?)`,
//           [user.id, bonusToPay, now, now]
//         );
//       } else {
//         console.log(
//           `[RUN ${runId}] Bonus already given today for user ${user.id}`
//         );
//       }
//     }
...
//     if (res) {
//       return res.status(200).json({
//         message: "Signup bonuses distributed successfully.",
//         processedUsers: users.length,
//       });
//     } else {
//       console.log(`[RUN ${runId}] Processed ${users.length} users.`);
//     }
//   } catch (error) {
