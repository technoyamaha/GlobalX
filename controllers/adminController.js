...
const processLeadershipReward = async (connection, depositedUserId) => {
  ...
  for (const uplineId of uplineIds) {
    ...
    const [existingUserRank] = await connection.query(
      `
      SELECT reward_id
    ...
    const currentRankLevel = existingUserRank[0]?.reward_id || 0;

    // ❌ Skip if already same or higher
    if (currentRankLevel >= highestRank.rankLevel) continue;

    // ✅ Insert new upgraded rank
    const [insertResult] = await connection.query(
      `
      INSERT INTO user_leadership_rewards
      (
        user_id,
        reward_id,
        rank_name,
      ...
      [
        uplineId,
        highestRank.rankLevel,
        highestRank.name,
        businessAmount,
        highestRank.oneTimeReward,
        highestRank.monthlyReward,
        now,
        now,
      ],
    );
    ...
    if (insertResult.affectedRows === 1) {
      ...
      await connection.query(
        ...
        WHERE id = ?
        `,
        [highestRank.oneTimeReward, uplineId],
      );
    }
  }
};
